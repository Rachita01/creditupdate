...
const ExcelCreation = ({ data }) => {
  const handleDownload = () => {
    ...
    const headers = ['PC Name','Date','Amount','Added On'];
    ...
    const dataArray = data.map(item => [item.pcname,item.date,item.amount,item.addedon]);
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Sheet1');
     ...
     XLSX.writeFile(workbook, `CreditUpdate.xlsx`, { bookType: 'xlsx', mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
